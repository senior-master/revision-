...
def generate_content(course, unit, topic):
    ...
    type_guidance = {
        "disease": " follow: Overview, Causes/Risk Factors, Pathophysiology (mechanism clearly explained stepwise), Clinical Manifestations (early vs late), Diagnosis (labs + clinical criteria), Treatment (medical + surgical if applicable), Nursing Management (prioritized interventions, rationales where needed), Prevention (primary/secondary/tertiary), Complications (acute vs chronic). Ensure clinical clarity and exam-focused phrasing.",
        "drug": "follow: Drug Class, Mechanism of Action (stepwise receptor/biochemical effect), Indications (primary + off-label if relevant), Adverse Effects (common vs severe), Contraindications, Nursing Responsibilities (before/during/after administration), Patient Education (clear, practical instructions), Dosage Notes (standard ranges if known, otherwise general guidance).",
        "organ": "follow: Overview, Gross Structure, Microscopic Anatomy (if applicable), Functions (physiological roles clearly separated), Physiology (how it works in systems), Clinical Relevance (what goes wrong in disease), Nursing Considerations (assessment + monitoring focus).",
        "procedure": "follow: Purpose/Indications, Equipment Needed (complete checklist style), Preparation (patient + environment), Steps (sequential, numbered, clear), Post-procedure Care (monitoring + safety), Complications (what can go wrong + signs), Nursing Responsibilities (critical thinking + prevention focus).",
        "theory": "follow: Overview, Author/Origin, Key Concepts (clearly defined), Components/Stages (structured breakdown), Principles (core rules or assumptions), Application to Nursing Practice (real clinical use cases), Limitations (weaknesses or criticisms).",
        "concept": "follow: Definition (precise), Principles, Classification/Types (if applicable), Importance, Nursing Relevance (clinical reasoning connection), Clinical Application (how it appears in real patient care).",
        "physiology":"follow: Overview, Mechanism/Process (step-by-step biological process), Regulatory Factors (hormonal/neural/chemical control), Clinical Significance (what abnormalities imply), Nursing Implications (assessment, monitoring, intervention relevance).",
        "diagnostic":"follow: Purpose, Principle, Procedure (clear stepwise explanation), Normal Values/Findings, Abnormal Findings (interpretation meaning), Nursing Responsibilities (before/during/after test care).",
        "diagnostic_tool": " follow: Overview, Principle (how tool works), Indications, Procedure (stepwise), Findings Interpretation (normal vs abnormal meaning), Nursing Responsibilities (safety + preparation + monitoring).",
        "equipment": "follow: Definition/Overview, Components (parts clearly listed), Indications (when used), How to Use (stepwise operational guide), Safety Considerations, Nursing Responsibilities (maintenance + patient safety).",
        "drug_class": "follow: Overview, Mechanism of Action (shared class mechanism), Examples (common drugs), Indications, Adverse Effects (class effects), Nursing Considerations (monitoring patterns + safety rules).",
        "healthcare_system": " follow: Definition, Structure (levels clearly separated), Functions, Key Components, Nursing Role (system-level responsibilities), Challenges (real-world constraints).",
        "communication_counselling": "follow: Definition, Principles, Types/Techniques (clearly separated), Barriers, Nursing Application (clinical scenarios), Therapeutic Use (patient outcomes + practice relevance).",
        "academic_professional_skill": "follow: Definition, Importance, Key Components, Steps/Process (actionable sequence), Nursing Application (how it is used in practice).",
    }
